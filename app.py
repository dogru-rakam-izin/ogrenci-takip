...
if giris_yap():
    ...
    with sekme2:
        ...
        if not df.empty:
            ...
            with f2:
                yil_listesi = ["Hepsi"] + sorted(list(set(df['Tarih'].str[-4:].tolist()))) if 'Tarih' in df.columns else ["Hepsi"]
                yil_sec = st.selectbox("Yıl Filtresi", yil_listesi)

            # Filtreleri Uygula
            if isim_ara:
                df = df[df['Ad Soyad'].str.contains(isim_ara, case=False, na=False)]
            if yil_sec != "Hepsi":
                df = df[df['Tarih'].str.endswith(yil_sec)]

            # Excel İndirme
            csv_data = df.to_csv(index=False, sep=';').encode('utf-8-sig')
            st.download_button(label="📥 Excel İndir", data=csv_data, file_name="Ogrenci_Takip.csv", mime="text/csv")
